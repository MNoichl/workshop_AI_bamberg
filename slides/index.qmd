...
|---|---|---|---|
| Wed, March 18, 2026 | 14:00-16:30 | Introductory Session | AI for Research |
| Thu, March 19, 2026 | 09:30-12:00 | Text Analysis | AI-powered text-analysis |
| Thu, March 19, 2026 | 13:00-15:30 | Programming | AI-based programming assistance |
...
* VSCode with Codex/Claude Code GUI pluging. Or Cursor.
...
* *10 Minute Break*
* *Exploration!*
* AI for research tasks.
* AI as scientific tool..
...
* Idea 1: Just enumerate all the words (BOW-Models, topic models) [OPEN IMAGE](#){.opens-modal data-modal-type="image" data-modal-url="images/BOW.jpeg"}
* But: Words have meaning!
* Idea 2: Use word vectors (E.g. Word2Vec) [OPEN IMAGE](#){.opens-modal data-modal-type="image" data-modal-url="images/W2V.jpeg"}
* But: Grammar!
* Idea 3: Split words into tokens (E.g. fastText) [OPEN IMAGE](#){.opens-modal data-modal-type="image" data-modal-url="images/tokens.jpeg"}
...
::: aside
[@vaswaniAttentionAllYouNeed2017]
:::
...
LLMs as **shoggoths**. Opaque, Lovecraftian amalgations of our common culture. [OPEN IMAGE](#){.opens-modal data-modal-type="image" data-modal-url="images/shoggoth_classic.jpeg"}
:::
...
# More terminology {layout="bullets" data-gp-kicker="Session 1 / Vocabulary" data-gp-section="Session 1"}

* **Context-window**: The number of tokens the model can see.
* **GPUs**: The chips AI runs on. Mostly NVIDIA: A10, RTX 3090, RTX 5090, A100, H100, H200, B100, etc. maybe Apple Silicon M1--M4?
* **Parameters**: Size of the model. Small: 3B, 7B, 13B; Medium: 70B, 175B; Large: 1-2T
* **Quants:** Size reduction of models by using lower bit integers. 8bit, 4bit, 2bi
* **MOE:** Mixture of Experts. A model that uses multiple experts to handle different parts of the input.
...
* [EXERCISE]{.gp-exercise-tag} How many r's in "strawberry"? [OPEN IMAGE](#){.opens-modal data-modal-type="image" data-modal-url="images/strawberry.png"}
* Try: Refrigerator + 5.2 Instant
...
* Hallucinations... [OPEN IMAGE](#){.opens-modal data-modal-type="image" data-modal-url="images/eat-rocks-google-ai-overviews.jpg.webp"}
* Remain! But more of a small model problem.
* ARC-AGI: A "fair" test for general intelligence? [OPEN TASK](#){.opens-modal data-modal-type="iframe" data-modal-url="https://arcprize.org/tasks/135a2760/"}
...
* Capabilities research suffers greatly from publication lag, cost of big models and bench-mark tuning/saturation.
...
* **Model Context Protocol (MCP)-servers:** Provide tools to LLMs in a uniform format.
...
* Compute is expensive! [OPEN GPU](#){.opens-modal data-modal-type="iframe" data-modal-url="https://turbomaxgpu.com/product/nvidia-h200-141gb/"}
...
* Probably not much worse than everything else we do, but it doesn't help either...
* Copyright & Commons
* Many specific issues: Labour displacement, data ethics, decision-making (e.g. health, warfare),...
* ...misinformation (online/legal/medical), mental health,...
* ...AGI X-risks, AI consciousness, etc.




<!-- # AI for research tasks {.gp-section-slide data-gp-section="Session 1"} -->


# General considerations {layout="bullets" data-gp-kicker="Session 1 / Research Use" data-gp-section="Session 1"}

* Prompt-engineering is largely over.
* But context matters a lot: Give it as much information as possible about what you want. 
* Try, evaluate, iterate. Work step by step.
* Don't trust it's confidence. Useful, but defeasible!
* Options to play with: GPT 5.4 Pro (very slow), Claude Opus 4.6 (fast), GPT-oss (local & open), Gemini, Kimi & Grok (via Cursor)
...
* Current SOTA detectors are **somewhat better** (E. g. [Pangram](https://pangram.com/)). 
* But still **unreliable enough** considering inductive risk!
* [EXERCISE]{.gp-exercise-tag} Ask your LLM a question & find tells. [OPEN SCREENSHOT](#){.opens-modal data-modal-type="image" data-modal-url="images/Screenshot_detection.png"}
* "It's not only X it's Y", "In practice....", "---"
* [EXERCISE]{.gp-exercise-tag} Can we fool Pangram? [Pangram API](#){.opens-modal data-modal-type="iframe" data-modal-url="https://maxnoichl-pangram-api-tester.hf.space"}
...
* Due to the "model-smell" I'd avoid autonomous rewrites
* Or do we already prefer AI? <a href="https://www.nytimes.com/interactive/2026/03/09/business/ai-writing-quiz.html" class="gp-modal-trigger" target="_blank" rel="noopener">NYT</a> [Econ](#){.opens-modal data-modal-type="iframe" data-modal-url="https://kevin-kloiber.com/choose-your-side.html"}
* But: Prompting for a list of edits, than picking the ones I want works well.
...
* Taking care of BibTeX and LaTeX.
...
* Heuristic review of code, maths  & statistics. [OPEN PAPER](#){.opens-modal data-modal-type="iframe" data-modal-url="images/non-standard-errors.pdf"}
* Ask: Find the crucial error!
* Sometimes surprising things turn up.
* Maybe a future in review pipelines. [OPEN PAPER](#){.opens-modal data-modal-type="iframe" data-modal-url="images/7815_In_Context_Watermarks_for.pdf"}
* [EXERCISE]{.gp-exercise-tag} Review a draft or paper.

::: aside
[@gaoNonstandardErrorsAIAgents2026,@liuInContextWatermarksLarge2026]
:::
...
* Autonomous research like **Deep Research** (OpenAI) can be very helpful.
* Limited by lack of open access (e.g. very good for computer science).
* Enrich citations (what am I missing?).
* [EXERCISE]{.gp-exercise-tag} Run a deep-research/large model web-search on a topic you know, and on one you don't.
...
# AI for annotation & labeling {layout="bullets" data-gp-kicker="Session 1 / Research Practice" data-gp-section="Session 1"}

* LLMs can do data-labelling on complex categories.
* Structured generation! [OPEN IMAGE](#){.opens-modal data-modal-type="image" data-modal-url="images/Structured%20annotation.png"}
* Serial queries are necessary: LLMs are lazy.
* Validate inter-rater agreement LLM & human, then scale up. Keep quotes!
* Usage via APIs or locally for sensitive data.
* Set spending limits, alerts & data-usage policies!
* [EXERCISE]{.gp-exercise-tag} Prompt your LLM to generate (only) JSON, to extract structured data from a text. [OPEN TEXT](#){.opens-modal data-modal-type="html" data-modal-url="media/data-annotation-exercise.html"}



# In silico social science {layout="bullets" data-gp-kicker="Session 1 / Research Practice" data-gp-section="Session 1"}

* Run experiments with AI instead of humans participants. [OPEN PAPER](#){.opens-modal data-modal-type="iframe" data-modal-url="images/in_silico.pdf"}
* Potentially extend to whole agent based models (ABMs).
* Expensive, and very controversial!
* Useful for prototyping data-collection & pre-registration?




# Discussion {layout="bullets" data-gp-kicker="Session 1 / Research Practice" data-gp-section="Session 1"}
...
* Programming: [Programming with AI deck](../slides/programming.html)
* Notebook: [Open in Colab](https://colab.research.google.com/github/MNoichl/workshop_AI_bamberg/blob/main/bamberg_text_analysis_AI.ipynb)
...
# Literature {layout="references" data-gp-kicker="Sources / 01" data-gp-section="Material"}
